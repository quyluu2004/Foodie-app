...
async function main() {
  try {
    console.log('🔍 Checking and fixing recipe titles...\n');
    ...
    // Show first 50 titles to check
    console.log('📝 First 50 recipe titles:');
    recipes.slice(0, 50).forEach((recipe, index) => {
      const hasNumber = /\s+\d+$/.test(recipe.title) || /\d+$/.test(recipe.title);
      const marker = hasNumber ? ' ⚠️' : '';
